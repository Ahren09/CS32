...
int main()
{
    ...
    Set a(3);
    ItemType items[7]={"one","two","three","four","five","six","seven"};
    for(int i=0;i<3;i++)
        assert(a.insert(items[i]));
    assert(!a.insert(items[3]));
    
    //Test the copy constructor
    Set b(a);
    assert(b.size()==3 && !b.insert(items[3]));
    
    //Test the assignment operator
    Set c(1000);
    c=a;
    assert(c.size()==3 && !c.insert("InsertionShouldFail"));
    
    //Test swap()
    Set d(40);
    for(int i=2;i<7;i++)
        assert(d.insert(items[i]));
    
    c.swap(d);
    assert(c.contains("seven") && c.size()==5 && d.contains("one"));
